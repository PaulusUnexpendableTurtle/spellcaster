...
function Card(triggers, actions, properties, owner, id) {
	...
	this.addProperty = function(property, gameContext, owner, appendFunc) {
		...
		gameContext.tryCall('property added', [prop, card, queue, owner],
		appendFunc);
	};
	...
	this.bind = function(target, card) {
		return {
			...
			triggers: (eventName, gameContext) =>
				gameContext.inherits(sourceString, eventName) && ((...arguments).includes(target) || !target),
			setNext: (f) => func = f,
			call: (gameContext) => {
				if (inList) {
					...
					() => {
						gameContext
						.tryCall('triggered', [trigger, card, queue, card.getOwner()],
						func);
					});
				} else
					func();
