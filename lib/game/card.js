function CardPool(cardData) {
	...
	this.get = (index, owner) => cardPool[index = ((index < 0 || index >= cardPool.length) ? 0 : index)].makeCard(owner, index);
};
...
function Card(triggers, actions, properties, owner, id) {
	...
	this.addProperty = function(property, gameContext, owner, appendFunc) {
		...
		//TODO: Property (to have public/private Forms)
		gameContext.tryCall('property added', [new Property(property), card, queue, owner],
		appendFunc);
	};
	...
	this.addTrigger = function(trigger, gameContext, owner, target, appendFunc) {
		...
		//TODO: trigger Forms
		gameContext.tryCall('trigger added', [trigger, card, queue, owner],
		appendFunc);
	};
	...
	this.addAction = function(action, gameContext, owner, appendFunc) {
		...
		//TODO: action Forms
		gameContext.tryCall('action added', [action, card, queue, owner],
		appendFunc);
	};

	//TODO: reveal(event), privateContents

	this.publicForm = (event) => {
		let prepare = { index: index };
		if (reveal(event)) {
			prepare.id = id;
			prepare.triggers = triggers;
	...
	this.privateForm = (event) => {
		let rev = reveal(event);
		return {
			id: id,
			index: index,
			triggers: rev ? privateTriggers() : triggers,
			actions: rev ? privateActions() : actions,
			properties: rev ? privateProperties() : properties
		};
	}
};
