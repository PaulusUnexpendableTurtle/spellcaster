...
function Card(triggers, actions, properties, owner) {
	...
	this.addTrigger = (trigger, gameContext, owner, target) => {
		...
		gameContext.triggersList.push(trueTrigger);
...
function getFunc(src, target, owner) {
	...
	if (param == '{action combo}')
		param = gameContext.getActionCombo();
	if (param == '{card combo}')
		param = gameContext.getCardCombo();
	...
	switch (func) {
		...
		case 'add trigger':
			return (gameContext) => target[0].addTrigger(new Trigger(param), gameContext, owner, target[1]);
		case 'add action':
