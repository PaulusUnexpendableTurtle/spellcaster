...
function Trigger(sourceString) {
	...
	{
		setQueue: (q) => queue = q,
		tryCall: (eventName, gameContext) => {
			if (gameContext.inherits(sourceString, eventName) && ((...arguments).includes(target) || target === null)) {
...
() => (target) => (gameContext) => wrap(target).bindActionTargets(gameContext);
