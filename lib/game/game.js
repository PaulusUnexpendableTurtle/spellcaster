...
function BindeeContainer(bindee, owner, func) {
	...
	this.apply = (gameContext) => {
		if (gameContext.inherits(bindee.string, 'lock')) {
			...
			playerOwner.on('sending targets', (...theArgs) => {
				...
				let targets = [];
				//TODO: getTarget(json of indexes)
				theArgs.forEach((targetData) => targets.push(getTarget(targetData)));

				//TODO: legal(bindee, targets)
				if (!legal(bindee.string, targets))
					return;
				permit = false;
