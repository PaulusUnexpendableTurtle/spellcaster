function GameContext(options) {
	...
	this.makeCard = (id, owner) => options.cardPool.get(id, owner, options.subSplitter);
	this.getSubSplitter = () => options.subSplitter;

	this.startHP = () => options.gameData.startHP;
	this.startDraw = () => options.gameData.startDraw;

	const eventTree = [];
	options.eventData.forEach((edge) => (eventTree[edge.from] = (eventTree[edge.from] || [])).push(edge.to));
	...
	this.tryCall = function(event, args, appendFunc) {
		...
		triggered.forEach((elem) => triggersList.splice(triggersList.indexOf(elem), 1));

		if (!triggered.length)
			return appendFunc();

		triggerCombo += triggered.length;
		...
		let sources = [];
		triggered.forEach((elem) => sources.push(...elem.getSources()));

		gameContext.tryCall('triggered', sources, () => {
	...
	var actionCombo = 0, cardCombo = 0, triggerCombo = 0, countDiscard = 0;
	this.getActionCombo = () => actionCombo;
	this.getCardCombo = () => cardCombo;
	this.getTriggerCombo = () => triggerCombo;
	this.getCountDiscard = () => countDiscard;

	this.increaseActionCombo = () => ++actionCombo;
	this.increaseTriggerCombo = () => ++triggerCombo;
	this.increaseDiscard = () => ++countDiscard;
