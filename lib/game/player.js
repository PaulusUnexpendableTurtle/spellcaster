function Player(socket, deck) {
	this.emit = (event) => socket.emit(event, ...arguments);
	this.getDeck = () => deck;
	var player = this;
	...
	var shuffleDeck = () => {
		var newDeck = [];
		while (deck.length) 
			newDeck.push(deck.splice(Math.floor(Math.random() * deck.length), 1));
		deck = newDeck;
	};
	var queues = [];
	...
	this.discardCard = (card, gameContext) => {
		if (card === null || gameContext.end())
			return false;
		grave.push(hand.splice(hand.indexOf(card), 1));
	...
	this.playCard = (cardIndex, queueIndex, gameContext) => {
		gameContext
		    .makeCard(hand.splice(cardIndex, 1), player)
